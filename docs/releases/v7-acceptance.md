...
- [ ] Runner 镜像已验证 `open-dataflow==1.0.10`，且 API/Worker 未安装 Runner-only Runtime；LLM/OCR 凭据仅经部署 Secret 注入。
...
| 11 | 依次验证 MinerU API/Local/Flash 与 DOCX/CSV 路由、Batch 自动选择 | 画布始终只显示 Document Parser / Text Cleaner / Semantic Chunker，部署变体和 Batch 不成为节点 | 待填写 |

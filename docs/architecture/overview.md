...
通用主链不依赖具体行业词表、文档内容或部署主体。医疗实体预设、医疗场景的机构 Deployment、qa_agent 接入及医疗样例是可选领域配置与集成，用来复用同一套 Source、ParsedDocument 校订审阅、Flow 输入快照、Knowledge Sink、AssetVersion 和 Routing 契约，不定义平台范围。新增能力默认先验证跨领域文档语义，再按需增加领域预设。
...
- 文档解析由独立 ParseJob 完成，产物是对象存储中的不可变 pending ParsedDocument；上传不触发 Flow 分块或知识生成。Markdown/Grid 校订批准会创建新的不可变 approved 修订。
- 需要分块的 Flow 显式包含 `document-input → document-chunker → execution_gate`。正式绑定任务复验 approved ParsedDocument，系统冻结输入 Snapshot 后自动继续。
...
- PDF 固定经 MinerU 3.4.4 `pipeline + auto + ch` 解析为 Markdown 与可逆 page/bbox Anchor；解析重试只重跑 ParseJob。
...
- Source 是逻辑资料；SourceVersion 以 `(source_id, sha256)` 唯一并引用全局内容寻址 Blob，只保存解析状态和当前 ParsedDocument 指针。FlowChunkSet 归属具体 Flow Revision，KnowledgeEvidence 可指向 FlowChunkRevision 或 ParsedDocument 行级证据。

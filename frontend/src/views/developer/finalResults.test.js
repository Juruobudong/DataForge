...
function run(keys = ['text', 'qa-question', 'graph:triple']) {
...
test('five templates and custom types use actual frozen sink order, not names or preview arrival order', () => {
  for (const keys of [['text'], ['qa-question'], ['qa-full'], ['graph:triple'], ['graph:semantic'], ['text', 'qa-question', 'graph:triple'], ['extension', 'graph:semantic', 'qa-full']]) {
...
test('typed cells preserve QA, graph entity/literal distinction and semantic relation labels', () => {
  ...
  assert.deepEqual(finalResultCells('qa-question', { data_json: { question: '问题', answer: '答案' } }), ['问题', '答案'])
  assert.deepEqual(finalResultCells('qa-full', { data_json: { question: '问题', answer: '答案' } }), ['问题', '答案'])
...
test('only upstream diagnostics affect each output and counts remain per stage', () => {
  const detail = run()
  detail.sink_previews = [preview('text'), preview('qa-question', 0)]
  detail.nodes = [{ node_id: 'g-qa-question', status: 'completed', metrics: { chunk_processing: processing('qa-question', 2, 3) } },
  ...
  detail.nodes[0].metrics.chunk_processing = processing('qa-question', 0, 5)
...
test('historical explicit extraction errors warn without fabricated counts; stdout/stderr alone do not', () => {
  const detail = run(['qa-full'])
  detail.sink_previews = [preview('qa-full')]
  detail.nodes = [{ node_id: 'g-qa-full', logs: [{ stream: 'stderr', message: 'QA_OUTPUT_INVALID: 提问方向必须为 JSON 字符串数组' }] }]

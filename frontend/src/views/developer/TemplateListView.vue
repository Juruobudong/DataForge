...
function definition() { return serializeDefinition(nodes.value, edges.value) }
...
async function load() {
  loading.value = true
  try { [templates.value, catalog.value, subflows.value, types.value] = await Promise.all([api.flowTemplates(), api.operatorCatalog(), api.flowSubgraphs(), api.knowledgeTypes()]) }
...
<template>
  <section class="template-page">
    ...
    <section v-else-if="activeTab==='catalog'" class="panel asset-table"><div class="panel-head"><div><h3>Operator Catalog</h3><p>仅 Canvas 暴露级别的受控逻辑节点可加入模板。</p></div><span class="badge blue">{{ catalog.length }} 个版本</span></div><table><thead><tr><th>算子</th><th>类别</th><th>暴露</th><th>输入端口</th><th>输出端口</th></tr></thead><tbody><tr v-for="item in catalog" :key="item.id"><td><b>{{ item.name }}</b><small>{{ item.code }} · v{{ item.version }}</small></td><td>{{ item.category }}</td><td><span class="badge" :class="item.exposure==='canvas'?'green':'amber'">{{ item.exposure }}</span></td><td><code>{{ JSON.stringify(item.input_ports) }}</code></td><td><code>{{ JSON.stringify(item.output_ports) }}</code></td></tr></tbody></table></section>
    <section v-else class="panel subflow-catalog"><div class="panel-head"><div><h3>可复用子图</h3><p>画布只保存子图引用，发布时递归展开为确定性快照。</p></div><span class="badge blue">{{ subflows.length }} 项</span></div><div class="subflow-grid"><article v-for="item in subflows" :key="item.id"><span>◈</span><div><b>{{ item.name }}</b><small>{{ item.code }} · r{{ item.revision }}</small><p>{{ item.definition?.nodes?.length || 0 }} 个内部节点 · {{ item.definition?.entry_node }} → {{ item.definition?.exit_node }}</p></div></article></div></section>

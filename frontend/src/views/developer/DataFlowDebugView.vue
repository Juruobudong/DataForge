...
function toggleReview(item) {
  ...
  if (selected.has(item.flow_chunk_review_snapshot_id)) selected.delete(item.flow_chunk_review_snapshot_id)
  else {
    const currentLibrary = (debugOptions.value?.review_inputs || []).find(value => selected.has(value.flow_chunk_review_snapshot_id))?.document_library_id
    if (currentLibrary && currentLibrary !== item.document_library_id) selected.clear()
    selected.add(item.flow_chunk_review_snapshot_id)
  }
...
function debugBody() {
  return {
    ...
    flow_chunk_review_snapshot_ids: inputSource.value === 'flow_chunk_review_snapshot' ? selectedReviewIds.value : [],
    sink_library_bindings: inputSource.value === 'flow_chunk_review_snapshot' ? { ...sinkBindings.value } : {},
  }
}

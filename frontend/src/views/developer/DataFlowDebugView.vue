...
async function load() {
  ...
  try {
    const [templateData, runData, catalogData, vectorData, capabilityData] = await Promise.all([
      api.flowTemplates(), api.flowRuns(), api.operatorCatalog({ include_internal: true }), api.vectorIndexes(), api.flowRunCapabilities(),
    ])
    templates.value = templateData; runs.value = runData; catalog.value = catalogData
    environment.value = vectorData; capabilities.value = capabilityData
    if (!selectedTemplateId.value && templates.value.length) selectedTemplateId.value = templates.value[0].id
    if (!selectedRunId.value && debugRuns.value.length) await inspectRun(debugRuns.value[0].id)
...
async function loadDebugOptions() {
  ...
  actionBusy.value = true; preflight.value = null
  try {
  ...
  } catch (e) { error.value = messageOf(e); debugOptions.value = null } finally { actionBusy.value = false }
}
function toggleReview(item) {
  ...
  selectedReviewIds.value = [...selected]; preflight.value = null
}
function debugBody() {
  return {
    ...
    source_review_snapshot_ids: selectedReviewIds.value,
    sink_library_bindings: { ...sinkBindings.value },
  }
}
async function runPreflight() {
  actionBusy.value = true; error.value = ''
  try { preflight.value = await api.debugRunPreflight(debugBody()) } catch (e) { error.value = messageOf(e) } finally { actionBusy.value = false }
}
async function createDebugRun() {
  ...
  try {
    ...
    const created = await api.createDebugRun({ ...debugBody(), idempotency_key: crypto.randomUUID() })
    drawerOpen.value = false; await load(); await inspectRun(created.id)
...
async function derive(mode) {
  ...
  try {
    ...
    const created = await api.createDerivedRun(selectedRunId.value, {
      ...
      parameter_overrides: { [selectedNode.value.node_id]: override }, idempotency_key: crypto.randomUUID(),
    })
    await load(); await inspectRun(created.id)
  } catch (e) { error.value = messageOf(e) } finally { actionBusy.value = false }
}
...
async function saveAsFlow() {
  ...
  try {
    actionResult.value = await api.saveDebugRunAsFlow(selectedRunId.value, {
      name: saveName.value, description: saveDescription.value, idempotency_key: crypto.randomUUID(),
    })
...
<template>
  <section class="debug-page">
    <div class="page-head"><div><h2>运行调试</h2><p>用真实审核输入运行不可变 Execution Snapshot；结果只生成 Artifact 与 Sink Diff Preview。</p></div><div class="page-actions"><span class="badge blue">Preview Only</span><button class="primary" :disabled="!selectedTemplate || loading" @click="openDebugDrawer">新建调试 Run</button><button :disabled="loading" @click="load">{{ loading ? '刷新中…' : '刷新' }}</button></div></div>
    <details class="environment"><summary>环境摘要 · {{ environment.managed_collections?.length || 0 }} Collections / {{ environment.profiles?.length || 0 }} Index Profiles</summary><div class="env-grid"><span v-for="item in environment.managed_collections || []" :key="item.id" class="badge" :class="item.status==='ready'?'green':'amber'">{{ item.collection_name }} · {{ item.status }}</span><span class="badge">协作式取消</span><span class="badge blue">调试不写正式知识</span></div></details>
    <div class="workbench">
    ...
    <div v-if="drawerOpen" class="overlay" @click.self="drawerOpen=false"><section class="drawer"><header><div><h3>新建调试 Run</h3><p>{{ selectedTemplate?.name }} · 真实 Runner · Preview Only</p></div><button @click="drawerOpen=false">关闭</button></header><label>Revision<select v-model="revisionKind" @change="loadDebugOptions"><option value="draft">当前草稿</option><option value="published">最新已发布 Revision</option></select></label><template v-if="debugOptions"><div class="revision-summary">r{{ debugOptions.revision.revision }} · {{ debugOptions.revision.status }} · {{ debugOptions.revision.authoring_mode }}</div><h4>输入文档</h4><p class="muted">可选择同一文档库中的多份当前审核冻结结果。</p><section v-for="group in groupedReviews" :key="group.id" class="review-group"><b>{{ group.name }}</b><label v-for="item in group.items" :key="item.source_review_snapshot_id" class="review-option"><input type="checkbox" :checked="selectedReviewIds.includes(item.source_review_snapshot_id)" @change="toggleReview(item)"><span>{{ item.filename }}<small>Review #{{ item.review_no }} · {{ item.chunk_count }} chunks</small></span></label></section><h4>输出预览目标</h4><label v-for="sink in debugOptions.sink_requirements" :key="sink.output_key">{{ sink.output_key }}<select v-model="sinkBindings[sink.output_key]" @change="preflight=null"><option value="">请选择 KnowledgeLibrary</option><option v-for="item in debugOptions.sink_options[sink.output_key] || []" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><div class="preview-policy"><b>Sink 模式：Preview Only</b><span>只计算 Diff，不写 KnowledgeLibrary。</span></div><p v-if="preflight" class="success">预检通过：{{ preflight.input_count }} 份输入 · {{ preflight.output_keys.join('、') }}</p></template><footer><button :disabled="actionBusy || !debugOptions" @click="runPreflight">运行预检</button><button class="primary" :disabled="actionBusy || !preflight" @click="createDebugRun">运行整个流程</button></footer></section></div>

...
const tab = ref('model'), models = ref([]), embeddings = ref([])
const editing = ref(null), form = ref(blankServiceForm('model')), error = ref(''), result = ref(null), busy = ref(false)
const rows = computed(() => tab.value === 'model' ? models.value : embeddings.value)
...
async function load() {
  error.value = ''
  try { [models.value, embeddings.value] = await Promise.all([api.modelServings(), api.embeddingServings()]) }
  catch (e) { error.value = e.message }
}
function openCreate() { editing.value = null; form.value = blankServiceForm(tab.value) }
function openEdit(item) { editing.value = item; form.value = editServiceForm(item, tab.value) }
async function save() {
...
function switchTab(value) { tab.value = value; openCreate() }
...
<template>
  <section class="model-services-page">
    ...
    <nav class="tabs"><button :class="{active:tab==='model'}" @click="switchTab('model')">大模型服务</button><button :class="{active:tab==='embedding'}" @click="switchTab('embedding')">Embedding 服务</button></nav>
    <div class="service-layout">
      <section class="panel service-list"><div class="section-head"><h3>{{ tab === 'model' ? '大模型服务' : 'Embedding 服务' }}</h3><button class="primary" @click="openCreate">新增</button></div>
        <table><thead><tr><th>服务</th><th>协议 / 模型</th><th>状态</th><th>测试结果</th><th>操作</th></tr></thead><tbody>
          <tr v-for="item in rows" :key="item.id"><td><b>{{ item.is_default ? '★ ' : '' }}{{ item.name }}</b><small><code>{{ item.serving_code }}</code></small></td><td>{{ item.serving_type || item.provider_type }}<small>{{ item.model_name }}<template v-if="item.dimension"> · {{ item.dimension }} 维</template></small></td><td><span :class="['badge', serviceStatus(item).tone]">{{ serviceStatus(item).label }}</span></td><td><template v-if="tab==='embedding'"><small>配置 {{ item.dimension }} / 实际 {{ item.last_observed_dimension ?? '—' }}</small></template><small>{{ item.last_check_latency_ms != null ? `${item.last_check_latency_ms} ms` : '—' }}</small></td><td class="actions"><button @click="openEdit(item)">编辑</button><button :disabled="busy" @click="action(item,'test')">测试</button><button v-if="!item.is_default" :disabled="busy || !item.is_enabled" @click="action(item,'default')">设为默认</button><button v-if="!item.is_default" :disabled="busy" @click="action(item,'toggle')">{{ item.is_enabled ? '停用' : '启用' }}</button><button @click="action(item,'references')">引用</button><button v-if="!item.is_default" class="danger" @click="action(item,'delete')">删除</button></td></tr>
        </tbody></table>
      </section>
      <form class="panel service-form" @submit.prevent="save"><h3>{{ editing ? '编辑' : '新增' }}{{ tab === 'model' ? '大模型' : ' Embedding' }} Serving</h3>
        ...
        <template v-if="tab==='embedding'"><label>Dimension<input v-model.number="form.dimension" type="number" min="1" required></label><label>Batch Size<input v-model.number="form.batch_size" type="number" min="1" required></label></template>
        ...
        <template v-if="tab==='model'"><label>Max Tokens<input v-model.number="form.max_tokens" type="number" min="1" required></label><label class="check"><input v-model="form.disable_thinking" type="checkbox">禁用 Thinking</label></template>

...
<template>
  <section>
    <nav class="breadcrumbs"><RouterLink to="/developer/flow-templates">模板</RouterLink><template v-for="item in trail" :key="`${item.id}-${item.revision}`"><span>›</span><RouterLink :to="`/developer/flow-templates/subgraphs/${item.id}/revisions/${item.revision}`">{{ item.name }} · r{{ item.revision }}</RouterLink></template><span>›</span><span>{{ detail?.name || '子图' }}</span><span>›</span><b>r{{ detail?.revision }}</b></nav>

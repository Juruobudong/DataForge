...
const EXPOSURE_LABELS = { canvas: '可直接使用', controlled: '受控使用', internal: '系统内部', disabled: '已禁用' }
...
const visible = computed(() => catalog.value.filter(item =>
  ...
  (!knowledge.value || item.knowledge_types?.includes(knowledge.value)) &&
    ...
    <div class="catalog-layout">
      <div class="panel catalog-list">
        <button v-for="item in visible" :key="item.id" class="operator-row" :class="{ active: selected?.id === item.id }" @click="selected = item">
          <div><b>{{ item.display_name_zh || item.name }}</b><small>{{ item.code }} · v{{ item.version }} · {{ item.category }}</small><p>{{ item.summary }}</p></div>
          <span class="badge" :class="item.exposure === 'canvas' ? 'green' : 'amber'">{{ EXPOSURE_LABELS[item.exposure] }}</span>
        </button>

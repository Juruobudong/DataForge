...
test('knowledge detail exposes review counts, batch actions and full vector publish gate', () => {
  ...
  assert.match(detail, /can_publish_after_auto_approval/)
  assert.match(detail, /auto_approve_pending_count/)
  assert.match(detail, /auto_approval_snapshot_digest/)
  assert.match(detail, /approve_pending: reviewRequired\.value/)
  assert.match(detail, /自动通过该知识库全部/)

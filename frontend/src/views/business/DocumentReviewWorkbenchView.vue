...
const editingId = ref(''), editContent = ref(''), showRechunk = ref(false)
let pollTimer = null
...
async function splitChunk(chunk) { const raw = prompt('请用空行分隔拆分后的文档块：', chunk.content); if (raw === null) return; const parts = raw.split(/\n\s*\n/).map(value => value.trim()).filter(Boolean); await run(() => api.splitSourceChunk(chunk.id, { parts, expected_revision_no: chunk.revision_no })) }
...
<template>
  <section>
    ...
    <div class="workbench">
      ...
      <section class="chunk-pane"><header><div><h3>Chunk 审核</h3><span>待审 {{ review?.counts?.pending_review || 0 }} · 通过 {{ review?.counts?.approved || 0 }} · 拒绝 {{ review?.counts?.rejected || 0 }}</span></div><div class="filters"><input v-model="keyword" placeholder="搜索 Chunk"><select v-model="filter"><option value="all">全部</option><option value="pending_review">待审</option><option value="approved">通过</option><option value="rejected">拒绝</option></select></div></header><div class="bulk"><span>已选 {{ selectedIds.length }}</span><button :disabled="selectedIds.length < 1 || busy" @click="batch('approve')">批量通过</button><button :disabled="selectedIds.length < 1 || busy" @click="batch('reject')">批量拒绝</button><button :disabled="selectedIds.length < 2 || busy" @click="mergeSelected">合并连续 Chunk</button></div><ChunkCard v-for="chunk in chunks" :key="chunk.id" :chunk="chunk" :selected="selectedIds.includes(chunk.id) || focusedChunkId === chunk.id" :editing="editingId === chunk.id" :edit-content="editContent" :busy="busy" @select="selectChunk" @focus="focusChunk" @edit="startEdit" @update:edit-content="editContent=$event" @save="saveChunk" @cancel="editingId=''" @split="splitChunk" @remove="removeChunk" @review="reviewChunk" @reopen="reopenChunk" /><p v-if="!chunks.length" class="empty">{{ prep.status === 'failed' ? '解析失败，尚无候选 Chunk。' : '当前筛选没有 Chunk。' }}</p><footer class="complete"><button class="primary" :disabled="busy || !canComplete" @click="completeReview">完成审核</button></footer></section>
    </div>

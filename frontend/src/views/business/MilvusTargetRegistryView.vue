...
const currentRevision = target => target?.current_revision || null
const candidateRevision = target => target?.candidate_revision || null
...
function healthDetails(revision, targetId) {
  if (!revision?.health_checked_at) {
    return startupTargetIds.has(targetId) ? '启动 30 秒后自动检查连接' : '尚未检查，可手动检查连接'
  }
  const checkedAt = new Date(revision.health_checked_at).toLocaleString('zh-CN', { hour12: false })
...
async function createTarget() {
  ...
  try {
    ...
    notice.value = value.current_revision?.verification_status === 'verified' ? 'Milvus 服务连接通过并已注册。' : '服务已保留，但连接失败，请修正地址后重试。'
  } catch (e) { error.value = e.message } finally { busy.value = false }
}
function startEdit(target) {
  editing.value = { id: target.id, name: target.name, milvus_url: candidateRevision(target)?.milvus_url || currentRevision(target)?.milvus_url || '', token: '', preserve_token: true }
}
async function saveEdit() {
  ...
  try {
    ...
    notice.value = value.candidate_revision?.verification_status === 'verification_failed'
...
async function checkHealth(target) {
  ...
  try {
    ...
    notice.value = value.current_revision?.health_status === 'healthy'
...
<template>
  <section class="milvus-registry-page">
    ...
    <template v-else>
      ...
      <section class="panel"><div class="panel-head"><div><h3>服务注册表</h3><p>{{ targets.length }} 个服务；默认知识写入目标：{{ instance?.authoring_milvus_target?.name || '未配置' }}</p></div></div><table><thead><tr><th>服务</th><th>当前 URI</th><th>配置与健康</th><th>中心引用</th><th>操作</th></tr></thead><tbody><tr v-for="target in targets" :key="target.id"><td><b>{{ target.name }}</b><br><small><code>{{ target.id }}</code></small></td><td><code>{{ currentRevision(target)?.milvus_url || '尚无已验证连接' }}</code><small v-if="currentRevision(target)">{{ currentRevision(target).token_configured ? ' · Token 已配置' : ' · 无 Token' }}</small><p v-if="candidateRevision(target)" class="candidate">候选：<code>{{ candidateRevision(target).milvus_url }}</code></p></td><td><div class="status-stack"><div><small>配置</small><span class="badge" :class="statusClass(currentRevision(target)?.verification_status || candidateRevision(target)?.verification_status)">{{ statusLabel(currentRevision(target)?.verification_status || candidateRevision(target)?.verification_status) }}</span></div><div v-if="currentRevision(target)"><small>当前健康</small><span class="badge" :class="healthClass(currentRevision(target).health_status)">{{ healthLabel(currentRevision(target).health_status) }}</span><small>{{ healthDetails(currentRevision(target), target.id) }}</small><small v-if="currentRevision(target).health_error" class="error-text">{{ currentRevision(target).health_error }}</small></div><div v-if="candidateRevision(target)"><small>候选配置</small><span class="badge" :class="statusClass(candidateRevision(target).verification_status)">{{ statusLabel(candidateRevision(target).verification_status) }}</span><small v-if="candidateRevision(target).verification_error" class="error-text">{{ candidateRevision(target).verification_error }}</small></div><div v-if="collectionChecks[target.id]" class="collection-result"><small>Collection</small><span class="badge" :class="collectionChecks[target.id].status==='available'?'green':'red'">{{ collectionChecks[target.id].status==='available' ? `共 ${collectionChecks[target.id].collection_count} 个` : '检查失败' }}</span><small v-if="collectionChecks[target.id].status==='available'">DataForge {{ collectionChecks[target.id].dataforge_collection_count }} 个<template v-if="collectionChecks[target.id].dataforge_collections?.length">：{{ collectionChecks[target.id].dataforge_collections.join('、') }}</template></small><small v-else class="error-text">{{ collectionChecks[target.id].error }}</small></div></div></td><td><span v-if="!references(target.id).length" class="muted">未绑定</span><div v-for="value in references(target.id)" :key="value">{{ value }}</div></td><td><div class="row-actions"><button @click="startEdit(target)">编辑</button><button v-if="currentRevision(target)" :disabled="busy" @click="checkHealth(target)">检查当前连接</button><button v-if="currentRevision(target)" :disabled="busy || collectionCheckId===target.id" @click="checkCollections(target)">{{ collectionCheckId===target.id ? '正在检查 Collection…' : '检查 Collection' }}</button><button v-if="candidateRevision(target)" :disabled="busy" @click="verifyTarget(target)">验证候选</button><button v-if="currentRevision(target)?.verification_status==='verified'&&instance?.authoring_milvus_target?.id!==target.id" :disabled="busy" @click="setAuthoring(target)">设为默认写入</button></div></td></tr><tr v-if="!targets.length"><td colspan="5" class="muted">暂无 Milvus 服务。</td></tr></tbody></table></section>
    </template>
...
.status-stack .badge{justify-self:start}
</style>

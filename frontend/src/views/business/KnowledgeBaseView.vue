...
const drawerOpen = ref(false), name = ref(''), type = ref('text'), graphMode = ref(null), creating = ref(false), deletingId = ref('')
const builtinCards = [
...
async function requestDelete(library) {
  ...
  try {
    ...
    if (!check.deletable) {
      ...
      if (bindings.length) window.alert('该知识库是文档库模板绑定的结果库，请先在文档库详情解绑对应模板后再删除。')
      else if (routes.length) window.alert('该知识库仍被项目路由引用，不能删除。')
      else window.alert('该知识库仍被引用，暂不能删除。')
      return
    }
    if (!window.confirm('将异步清理该知识库的 V7 Partition，确认继续？')) return

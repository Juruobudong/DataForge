...
async function createProjectTask() {
  try {
    ...
    await load(); newProjectTaskId.value = created.id; notice.value = `业务任务「${created.name}」已创建，请继续配置运行任务。`
  } catch (e) { error.value = e.message }
}
async function createDeploymentTask() {
  try {
    ...
    await loadDeployment(); notice.value = '运行任务已创建。'
  } catch (e) { error.value = e.message }
}
...
<template>
  <section>
    <div class="page-head"><div><h2>项目发布</h2><p>选择发布目标和环境，配置运行任务、知识范围并生成独立发布版本。</p></div><div class="page-actions"><span class="badge blue">{{ local ? '机构本地' : '中心控制面' }}</span><button v-if="central" class="primary" @click="showCreateProject=!showCreateProject">{{ showCreateProject?'取消新增':'新增项目' }}</button></div></div>

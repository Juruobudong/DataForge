...
<template>
  <article class="input-node" :class="{ selected }">
    <header><span class="icon">→</span><div><em>INPUT</em><b>已审核文档块</b><small>运行时自动绑定 · SourceReviewSnapshot</small></div></header>
    <section><TypedHandle v-for="(spec, port) in data.meta.outputs" :key="port" :port="port" :spec="spec" :definition="data.definition" :node-kind="data.meta.kind" direction="output" /></section>
    <footer>流程输入 · 无需 Incoming Edge</footer>
  </article>
</template>

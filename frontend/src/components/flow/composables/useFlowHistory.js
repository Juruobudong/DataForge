...
export function useFlowHistory(nodes, edges, limit = 40) {
  const past = ref([]), future = ref([])
  const canUndo = computed(() => past.value.length > 0)
  const canRedo = computed(() => future.value.length > 0)
  const snapshot = () => cloneGraph(nodes.value, edges.value)
  function remember() {
    past.value.push(snapshot())
    if (past.value.length > limit) past.value.shift()
    future.value = []
  }
  function restore(value) { nodes.value = value.nodes; edges.value = value.edges }

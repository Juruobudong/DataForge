...
export const dataflowOperators = [
  ...
  driver: 'dataflow', executor: code === 'HashDeduplicateFilter' || code === 'MinHashDeduplicateFilter' ? 'dataflow-storage' : 'dataflow-llm',
...
export const nativeQaOperator = { code: 'qa-extractor', name: 'QA Extractor', display_name_zh: '问答提取器', version: 1, source: 'dataforge', catalog_group: 'dataforge', driver: 'builtin', executor: 'dataforge-native', category: 'content-processing', kind: 'operator', surfaces: ['standard-template', 'advanced-canvas'], dependency_status: { status: 'ready' } }
...
const chunking = { code: 'chunking', name: '文档切分', configurable: true, operators: [{ code: 'document-chunker', node_id: 'chunker', name: 'Document Chunker', display_name_zh: '文档切分', source: 'dataflow' }], config_schema: { type: 'object', properties: {
  split_method: { type: 'string', title: '切分方式', enum: ['recursive', 'sentence'] }, chunk_size: { type: 'integer', title: '目标大小' }, chunk_overlap: { type: 'integer', title: '片段重叠' },
} } }
const inputReview = { code: 'input_review', name: '自动冻结输入', configurable: false }

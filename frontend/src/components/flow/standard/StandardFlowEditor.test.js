...
beforeEach(() => {
  ...
  api.resolveStandardFlow.mockResolvedValue({ resolved_operators: [
    { node_id: 'input', kind: 'operator', code: 'reviewed-source-chunk-input', display_name_zh: '已审核来源切片', name: 'Reviewed SourceChunk Input', version: 3, source: 'dataforge', catalog_group: 'dataforge', driver: 'builtin', executor: 'dataforge-native', input_ports: {}, output_ports: { output: { artifact_type: 'source_chunk_set' } }, parameters: {}, locked: true },
    { node_id: 'qa', kind: 'operator', code: 'qa-extractor', display_name_zh: '问答生成器', name: 'QA Extractor', version: 1, source: 'dataforge', catalog_group: 'dataforge', driver: 'builtin', executor: 'dataforge-native', stage_label: '问答生成', uses_llm: true, input_ports: { input: { artifact_type: 'source_chunk_set' } }, output_ports: { output: { artifact_type: 'candidate:qa' } }, parameters: { questions_per_chunk: 1 }, output_key: 'qa', locked: true },
...
describe('Standard business stages follow the managed contract', () => {
  ...
  it('text has input, mapping and sink without generation or pretend quality', async () => {
    ...
    expect(wrapper.findAll('h3').map(item => item.text())).toEqual(['输入', '文本知识映射', '输出知识'])
  ...
  it.each(['standard-qa', 'standard-graph-triple', 'standard-graph-semantic', 'standard-multi'])('%s keeps generation controls separate from quality', async code => {
    await render(code)
    const count = code === 'standard-qa' ? 3 : code === 'standard-multi' ? 5 : 4

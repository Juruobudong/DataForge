...
const emit = defineEmits(['before-change', 'select-node', 'select-edge', 'connection-error', 'add-definition'])
const root = ref(null)
const activeConnection = ref(null)
provide('dataforge-active-connection', activeConnection)
const { fitView, screenToFlowCoordinate, setCenter } = useVueFlow('dataforge-template-flow')
...
function connect(connection) { emit('before-change'); if (!addTypedEdge(connection)) return }
...
<template>
  <div ref="root" class="flow-canvas" @dragover.prevent @drop="drop">
    <VueFlow id="dataforge-template-flow" v-model:nodes="nodes" v-model:edges="edges"
      ...
      nodes-draggable nodes-connectable elements-selectable zoom-on-scroll pan-on-drag selection-on-drag
      ...
      @pane-click="emit('select-node', null)" @node-drag-start="emit('before-change')">
      <Background :variant="BackgroundVariant.Dots" :gap="18" :size="1.1" pattern-color="#cad3df" bg-color="#f7f9fc" />
      <Controls position="bottom-right" />
      <MiniMap position="bottom-left" :pannable="true" :zoomable="true" :node-stroke-width="2" node-color="#dce8fa" mask-color="rgba(238,242,247,.72)" />

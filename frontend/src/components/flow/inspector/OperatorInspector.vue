...
<template>
  <aside class="operator-inspector" v-if="operator">
    <header><div><small>{{ operator.category }} / {{ operator.subcategory || '通用' }}</small><h3>{{ operator.display_name_zh || operator.name }}</h3><code>{{ operator.code }}@{{ operator.version }}</code></div><span class="badge blue">{{ exposure }}</span></header>
    ...
    <div v-else class="inspector-body"><h4>Input Ports</h4><pre>{{ JSON.stringify(operator.input_ports || {}, null, 2) }}</pre><h4>Output Ports</h4><pre>{{ JSON.stringify(operator.output_ports || {}, null, 2) }}</pre><h4>Parameter Schema</h4><pre>{{ JSON.stringify(operator.parameter_schema || {}, null, 2) }}</pre></div>
  </aside>
</template>

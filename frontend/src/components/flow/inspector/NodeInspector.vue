...
<template>
  <aside class="node-inspector">
    ...
    <template v-if="node">
      ...
      <section v-if="node.data.meta.kind === 'operator'" class="inspector-body"><p>来源 {{ node.data.meta.source || 'dataforge' }} · 分组 {{ node.data.meta.catalogGroup || 'dataforge' }} · v{{ node.data.definition.operator_version || node.data.meta.version }}</p><p>Driver {{ node.data.meta.driver || '未声明' }} · Executor {{ node.data.meta.executor || '未声明' }}</p><p v-if="node.data.meta.dependencyStatus && node.data.meta.dependencyStatus.status !== 'ready'" class="error">{{ node.data.meta.dependencyStatus.reason || '算子依赖不可用' }}</p><label v-if="operatorVersions.length > 1">执行版本<select aria-label="算子执行版本" :value="node.data.definition.operator_version || node.data.meta.version" @change="emit('change-operator-version', Number($event.target.value))"><option v-for="item in operatorVersions" :key="item.version" :value="item.version">v{{ item.version }} · {{ item.source }}</option></select></label></section>
      <section v-if="tab==='parameters'" class="inspector-body">
        <details v-if="node.data.meta.runtimeRequirements?.adapted_behavior" class="adapted-contract"><summary>技术详情 · DataForge 适配契约</summary><p>{{ node.data.meta.runtimeRequirements.adapted_behavior }}</p><p>上游实现：{{ node.data.meta.runtimeRequirements.implementation }}</p><p>Adapter：{{ node.data.meta.runtimeRequirements.adapter_version }}</p></details>
        ...
        <template v-else>
          <OperatorParameterForm v-if="editable" :schema="parameterSchema" :model-value="params" :entity-types="entityTypes" :evaluation-nodes="evaluationNodes" :deduplication-nodes="deduplicationNodes" @update:model-value="updateParameters" />

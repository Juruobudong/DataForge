<script setup>
defineProps({ chunk: { type: Object, required: true }, selected: Boolean, editing: Boolean, editContent: String, busy: Boolean })
defineEmits(['select', 'focus', 'edit', 'update:editContent', 'save', 'cancel', 'split', 'remove', 'review', 'reopen'])
</script>
...
<template>
  <article class="chunk-card" :class="[chunk.review_status, { focused: selected }]" @click="$emit('focus', chunk)">
    ...
    <textarea v-if="editing" :value="editContent" rows="8" @input="$emit('update:editContent', $event.target.value)"></textarea>

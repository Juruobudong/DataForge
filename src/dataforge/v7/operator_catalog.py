...
def seed_catalog(session):
    ...
    for item in entries.values():
        validate_runtime_requirements(item.get("runtime_requirements"))
...
def technical_projection(definition, catalog):
    ...
    for node in definition.get("nodes", []):
        ...
        nodes.append({
            ...
            "driver": runtime["driver"] if node.get("kind") == "operator" else None,
            "executor": runtime["executor"] if node.get("kind") == "operator" else None,

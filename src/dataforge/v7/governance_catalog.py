...
RULE_SCHEMA = {"type": "array", "title": "保留条件（全部满足）", "minItems": 1, "maxItems": 32,
    ...
    "required": ["field", "operator"], "properties": {
        "field": {"type": "string", "enum": ["text", "question", "answer", "length", *SCORES]},
...
GOVERNANCE_SPECS = {
    ...
    "GeneralFilter": ("通用条件过滤器", "质量治理", "core_text", "conditions", "按全部满足的业务条件保留记录；支持正文与QA评分，不接受Python或表达式。", {"rules": RULE_SCHEMA}),
}
...
def extend_catalog(entries, package, package_version, package_digest, lock_digest):
    ...
    for code, (name, category, namespace, adapter, description, properties) in GOVERNANCE_SPECS.items():
        item = deepcopy(base)
        uses_llm = adapter in {"multihop", "evaluate"}
        ...
        item["runtime_requirements"] = {"provider": "dataflow", "executor": "dataflow-llm" if uses_llm else "dataflow-storage",
            ...
            "data_behavior": "生成匿名化副本，原始Evidence保留" if adapter == "anonymize" else "生成问答" if adapter == "multihop" else "附加评分，不改变正文" if adapter == "evaluate" else "只保留或过滤记录",

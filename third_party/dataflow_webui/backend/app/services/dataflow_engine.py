    ...
    def run(self, pipeline_config: Dict[str, Any], task_id: str, execution_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if execution_results:
                final_index = execution_results[-1].get("index", len(execution_results) - 1)
                shared_output = os.path.join(settings.CACHE_DIR, f"dataflow_cache_step_{final_index}.jsonl")
                if os.path.isfile(shared_output):
                    ...
                    shutil.copy2(shared_output, final_output)

...
| `/api/vector-storage/*` 实时库存与校验 | `DONE` | overview/list/detail 聚合实时 Milvus 与 MySQL；verify 才全量 digest 并保存最近结果；load/release 每次复验 ownership/Contract/资产映射；本命名空间无 DELETE。 |

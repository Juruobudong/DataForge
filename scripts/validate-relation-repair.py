"""Real-model acceptance for the default joint graph extractor in disposable SQLite.
...
RESULT_RE = re.compile(
    r"GRAPH_JOINT_RESULT: source_chunk_id=(\S+) entities=(\d+) relations=(\d+) "
    r"model_calls=(\d+) repair_attempts=(\d+) zero_reason=(\S+)"
)
...
def main():
    ...
    if args.rounds != 3:
        parser.error("joint extraction acceptance requires exactly three consecutive rounds")
    ...
    with tempfile.TemporaryDirectory(prefix="dataforge-joint-graph-") as directory:
        ...
        try:
            ...
            for index in range(3):
                ...
                created = store.create_debug_run(
                    ...
                    idempotency_key=f"joint-graph-real-{index}",
                )
                store.claim_debug_run(f"joint-graph-real-{index}")
                started = time.monotonic()
                result = execute_debug_run(store, None, created["id"])
                detail = store.flow_run_detail(created["id"])
                node = next(value for value in detail["nodes"] if value["node_id"] == "extract-graph:triple")
                stat = node["metrics"].get("chunk_processing", [{}])[0]
                joint = node["metrics"].get("joint_extraction", {})
                log_text = "\n".join(value.get("message", "") for value in node.get("logs", []))
                logged = {match[1]: {"entity_count": int(match[2]), "relation_count": int(match[3]),
                                     "model_calls": int(match[4]), "repair_attempts": int(match[5]), "zero_reason": match[6]}
                          for match in RESULT_RE.finditer(log_text)}
                with store.sessions() as session:
                    node_run = session.scalar(select(FlowNodeRun).where(
                        FlowNodeRun.flow_run_id == created["id"], FlowNodeRun.node_id == "extract-graph:triple"))
                ...
                passed = (result["status"] == "completed" and node["operator_version"] == 1
                          ...
                          and all(value["model_calls"] in {1, 2} for value in by_chunk.values())
                ...
                entry = {"round": index + 1, "passed": bool(passed), "run_id": created["id"],
                         ...
                         "joint_extraction": joint, "candidate_count": len(candidates),
    ...
    if not report["passed"]:
        raise SystemExit("joint graph real-model acceptance failed; inspect report")
